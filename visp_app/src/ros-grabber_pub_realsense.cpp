//ros-grabber_pub_realsense.cpp (pub version) for use with usb connect Intel RealSense D435i 
...
int main(int argc, char **argv)
{
    ...
    std::cout << "visp_app ros-grabber_pub_realsense" << std::endl;
    //int opt_barcode = 0; 
    ...
    g.setImageTopic("/camera/color/image_raw");//! [Setting camera topic]
    if (opt_use_camera_info) 
    {
      g.setCameraInfoTopic("/camera/camera_info");//! [Setting camera info]
      g.setRectify(true);
    }
    g.open(I);//! [Opening]
